...
void update_position_encoders( BigStruct *all_struct, odometer_data *odo_data) 
{
	...
	spi_transfer(all_struct->fd1, left, 5);
	spi_transfer(all_struct->fd1, right, 5);
    ...
    if (all_struct->rob_pos->theta > 2*PI)
    {
        all_struct->rob_pos->theta =  fmod(all_struct->rob_pos->theta, 2*PI);
    }
    while (all_struct->rob_pos->theta < 0)
    {
        all_struct->rob_pos->theta += 2*PI;
    }
